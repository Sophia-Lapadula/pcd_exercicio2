...
int iterations, N, n_cels_totais, numTh;
struct timeval start_Reduction, end_Reduction;
...
void getLivingCells(int** grid){
    int i, j;
    gettimeofday(&start_Reduction, NULL);
    #pragma omp parallel for num_threads(numTh) \
                            private(i, j) \
                            shared (N, grid)\
                            reduction(+: n_cels_totais)
    for(i=0;i<N;i++){
        for(j=0;j<N;j++){
            n_cels_totais += grid[i][j]; 
        }
    }
    gettimeofday(&end_Reduction, NULL);
}
...
int main() {
    ...
    float tempo_Reduction = (float) ((end_Reduction.tv_sec-start_Reduction.tv_sec)+(end_Reduction.tv_usec-start_Reduction.tv_usec));
    printf("Tempo de execucao parte pragma reduction: %f milissegundos.\n", tempo_Reduction);
    printf("\n\n");
